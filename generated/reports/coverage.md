# Registry Coverage Report

Generated for registry state: 2026-08-24T13:14:55.511915Z

| Harness | Verified capabilities | Candidate | Unknown | Releases |
|---|---:|---:|---:|---:|
| Antigravity CLI | 28 | 0 | 0 | 37 |
| Claude API SDK — Python | 3 | 0 | 0 | 42 |
| Claude API SDK — TypeScript | 3 | 0 | 0 | 45 |
| Claude Agent SDK — Python | 9 | 0 | 0 | 119 |
| Claude Agent SDK — TypeScript | 9 | 0 | 0 | 126 |
| Claude Code | 34 | 2 | 0 | 126 |
| Codex SDK — Python | 6 | 0 | 0 | 5 |
| Codex SDK — TypeScript | 3 | 1 | 0 | 291 |
| Gemini CLI | 12 | 0 | 0 | 136 |
| GitHub Copilot CLI | 4 | 0 | 0 | 162 |
| Hermes Agent | 19 | 2 | 0 | 30 |
| OpenAI API SDK — Node | 3 | 0 | 0 | 22 |
| OpenAI API SDK — Python | 3 | 0 | 0 | 30 |
| OpenAI Agents SDK — JavaScript/TypeScript | 9 | 1 | 0 | 31 |
| OpenAI Agents SDK — Python | 9 | 1 | 0 | 36 |
| OpenAI Codex | 30 | 1 | 0 | 301 |
| OpenCode | 16 | 3 | 0 | 98 |
| Pi Agent Harness | 5 | 0 | 0 | 61 |
| Qwen Code | 5 | 3 | 0 | 272 |
| goose | 7 | 4 | 0 | 19 |

## Interpretation

A high release count does not imply comprehensive capability coverage. Release collection and documentation-derived capability verification are separate pipelines by design.
